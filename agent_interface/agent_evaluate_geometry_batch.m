function agent_evaluate_geometry_batch(config_path, result_path)
%AGENT_EVALUATE_GEOMETRY_BATCH Dedicated JSON batch interface for array design.
...
try
    ...
    if ~isfile(config_path)
        error('hermes:ArrayDesignConfigNotFound', 'Configuration file does not exist: %s', config_path);
    end
    try
        config = jsondecode(fileread(config_path));
    catch exception
        error('hermes:ArrayDesignJsonParseError', 'Could not parse configuration JSON: %s', exception.message);
    end
    required = ["batch_id", "design_task_id", "target_mm", "focus_tolerance_mm", ...
        "roi_radius_mm", "roi_half_depth_mm", "geometries"];
    for field = required
        if ~isfield(config, field)
            error('hermes:ArrayDesignMissingField', 'Required field %s is missing.', field);
        end
    end
    ...
    addpath(fullfile(project_root, 'matlab_core', 'array_design'), '-begin');
...
if bytes_written <= 0 || close_status ~= 0
    ...
    error('hermes:ResultWriteError', 'Could not finish writing result JSON: %s', result_path);
end
[moved, move_message] = movefile(temporary_path, result_path, 'f');
if ~moved
    if isfile(temporary_path), delete(temporary_path); end
    error('hermes:ResultWriteError', 'Could not publish result JSON: %s', move_message);
end
end
